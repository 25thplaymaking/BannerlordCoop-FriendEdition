...
## 3. AI parties never initiate an encounter on a player (bandits ignore you)
...
## 4. Replication traffic peaks at ~6 MB/s to a single client
...
## 5. Host: ~4.5 s game-thread stall every 5 minutes (autosave)
...
## 6. Host: the headless map scene reports the wrong terrain size
...
## 7. EoE data: malformed settlement entries
