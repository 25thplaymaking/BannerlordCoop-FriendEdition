...
All affected production and test projects compile in Release with zero errors. The normal VSTest host repeatedly fails to connect after discovery in this environment (IPv4 loopback is broken machine-wide; `dotnet test`/vstest never reaches its testhost), so runtime tests are executed with the xunit 2.9.3 in-process console runner (`xunit.console.dll ... -noshadow -parallel none`) run directly against each test assembly's build output instead. Under that runner the `GameInterface.Tests.Services.WorkshopMods` namespace executes 233 tests with 2 failures, and `E2E.Tests.Services.WorkshopMods` executes 63 tests with 0 failures, both stable across repeated consecutive runs (5+) with identical counts.
