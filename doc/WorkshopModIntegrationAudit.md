...
All affected production and test projects compile in Release with zero errors. The focused test assemblies also compile, but the normal VSTest host repeatedly fails to connect after discovery in this environment, so the executed runtime-test count is zero and no live Bannerlord E2E certification is claimed. Package publication remains blocked because Steam reports a newer Fourberie manifest than the audited installed copy; the updated bytes must be installed, fingerprinted, reviewed, and revalidated before a distributable, commit, release, or deployment is approved.
