...
public class VillageNeedsCraftingMaterialsIssueTests : IDisposable
{
    ...
    private void CreateIssueOnServer(string ownerId)
    {
        Server.Call(() =>
        {
            ...
            var pid = new PotentialIssueData(
                ...
                {
                    // Registered here, inside the factory delegate: the creation postfix broadcast fires
                    // synchronously as part of this same CreateNewIssue call, before control returns below.
    ...
    public void QuestOwnershipGate_BlocksTurnInForAnyoneOtherThanTheRecordedOwner_EvenWithTheMaterialsInHand()
    {
        ...
        // Fresh Call: this harness's static swap doesn't restore Campaign.Current back to the Server's own
        // after StartIssueQuest's nested per-client broadcast dispatch unwinds - only a new Call's entry does.
        Server.Call(() =>
        {
