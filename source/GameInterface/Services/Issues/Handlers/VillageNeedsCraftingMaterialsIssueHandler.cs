...
internal class VillageNeedsCraftingMaterialsIssueHandler : IHandler
{
    ...
    private void Handle_VillageCraftingIssueQuestAcceptTriggered(MessagePayload<VillageCraftingIssueQuestAcceptTriggered> payload)
    {
        ...
        {
            // No amount/reward/ControllerId here: none of this client's locally-rolled values can be trusted
            // as authoritative - the server re-derives its own via ReplayQuestAccepted below.
        ...
        MessagePayload<RequestVillageCraftingIssueAlternativeSolutionCompletion> payload)
    {
        ...
        GameThread.RunSafe(() =>
        {
            ...
            // Only the recorded owner may trigger their own alternative-solution completion.
            if (!VillageNeedsToolsIssueOwnership.TryGetOwnerControllerId(owner, out var recordedControllerId) ||
