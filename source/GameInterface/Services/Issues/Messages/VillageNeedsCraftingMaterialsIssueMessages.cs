...
// Deliberately separate wire types from Tools' identically-shaped equivalents: sharing them would make
// VillageNeedsToolsIssueHandler's own `owner.Issue is VillageNeedsToolsIssue` check misfire a rejection back
// to a client whose accept was actually for (and correctly handled by) this other issue type.
//
// Teardown IS safely shared unchanged - VillageIssueFinalizeReason/VillageIssueFinalizedTriggered/
// RequestVillageIssueRemoved/NetworkVillageIssueRemoved (in VillageNeedsToolsIssueMessages.cs) carry no
// Tools-specific constraint.
...
// Local-only trigger: the owner's own HourlyTick determined its alternative solution is ready to complete but
// this peer isn't the server, so CompleteIssueWithAlternativeSolution can't run here (see
// AlternativeSolutionCompletionRunner's own doc comment for why) - the handler turns this into a real network
// request instead.
