...
internal class SettlementSync : IAutoSync
{
    public SettlementSync(AutoSyncRegistry AutoSyncRegistry)
    {
        ...
        // NOT replicated: the four Nearby*Intensity heuristics are server-only AI inputs, and they were
        // measured at ~8,900 messages per 10 s on a three-player session — about 12% of ALL replication
        // traffic — for nothing a client can observe.
        //
        // Every reader in the game is AI scoring or save serialisation: DefaultTargetScoreCalculatingModel
        // (settlement target scores), MobilePartyAi (behaviour scores), PoliticalStagnationAndBorderIncident-
        // CampaignBehavior (which is also what churns them), and AutoGeneratedSaveManager. The names do not
        // appear anywhere in SandBox.ViewModelCollection, SandBox.GauntletUI or
        // TaleWorlds.CampaignSystem.ViewModelCollection, so no UI reads them. The host owns all AI for parties
        // a client does not control, and the host writes the save, so the client never needs the value.
        //
        // They were already flagged chatty with coalesce: true, which cannot help here — coalescing merges
        // repeats of the SAME settlement between flushes, and the volume is thousands of DISTINCT settlements.
        // A joining client still receives them in the save transfer, and the behavior above recomputes them
        // locally, so nothing is left uninitialised.
        //
