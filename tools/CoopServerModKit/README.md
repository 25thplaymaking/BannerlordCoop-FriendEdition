...
|---|---|---|---|
| `StartupHook.cs` (+ `coophook.csproj`) | `dotnet build -c Release` (net6.0) | `coophook.dll` | `~/bannerlord-coop/server/coophook.dll`, referenced via `DOTNET_STARTUP_HOOKS='Z:\…\coophook.dll'`; installs the exact four-class v1.4.8 dedicated gameplay allowlist before module filtering; DismembermentPlus remains client-only because its presentation bootstrap requires WinForms |
